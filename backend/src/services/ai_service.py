...
class AIService:
    ...
    def __init__(self):
        ...
        self.patterns = {
            ...
            'add_todo': [
                r'(?:add\s+)?title\s+(.+?),?\s+add\s+description\s+(.+)',  # Pattern for "add title [title], add description [description]"
                r'add\s+(.+?)\s+with\s+description\s+(.+)',  # Pattern for "add [title] with description [description]" - highest priority
                r'add\s+(.+?)\s+and\s+description\s+(.+)',  # Pattern for "add [title] and description [description]" - second priority
                r'add\s+(.+?)\s*,\s*(.+)',  # Pattern for "add [title], [description]" - comma separation
    ...
    def _initialize_assistant(self) -> Assistant:
        ...
        try:
            ...
            assistant = self.openai_client.beta.assistants.create(
                ...
                When users provide both a title and description, make sure to extract both and pass them to the add_todo tool.
                Supported formats for title and description:
                - "add [title] with description [description]"
                - "add [title] and description [description]"
                - "add [title], [description]" (comma-separated)
                - "add title [title], add description [description]"
        ...
        for intent, patterns in self.patterns.items():
            for pattern in patterns:
                ...
                if match:
                    ...
                    if intent == 'add_todo':
                        # Check if this is the pattern with both title and description
                        try:
                            # Check for the two-group patterns (title and description)
                            if match.groups() and len(match.groups()) >= 2:
                                title_match = match.group(1)
                                description_match = match.group(2)

                                # Handle the comma-separated format specially
                                pattern_str = str(match.re.pattern)
                                if r'add\s+(.+?)\s*,\s*(.+)' in pattern_str:
                                    # For comma-separated, the first part might include "add" which we need to remove
                                    title_part = title_match.strip()
                                    for prefix in ['add ', 'Add ']:  # Handle both lowercase and capitalized
                                        if title_part.startswith(prefix):
                                            title_part = title_part[len(prefix):].strip()

                                    entities['title'] = title_part
                                    entities['description'] = description_match.strip()
                                else:
                                ...
                                # Additional check for title/description in the original input
                                # Look for patterns like "add [title] with description [description]" even if not captured by the regex
                                user_input_lower = user_input.lower()
                                if ' with description ' in user_input_lower:
                                    parts = user_input_lower.split(' with description ', 1)
                                    if len(parts) == 2:
                                        # Extract title from the first part (after removing command words)
                                        title_part = parts[0].strip()
                                        # Remove common command prefixes
                                        for prefix in ['add ', 'create ', 'new ']:
                                            if title_part.startswith(prefix):
                                                title_part = title_part[len(prefix):].strip()

                                        description_part = parts[1].strip()
                                        entities['title'] = title_part
                                        entities['description'] = description_part
                                elif ' and description ' in user_input_lower:
                                    parts = user_input_lower.split(' and description ', 1)
                                    if len(parts) == 2:
                                        # Extract title from the first part (after removing command words)
                                        title_part = parts[0].strip()
                                        # Remove common command prefixes
                                        ...
                                        entities['title'] = title_part
                                        entities['description'] = description_part
                                elif ',' in user_input and 'add ' in user_input_lower:
                                    # Handle "add [title], [description]" format when not caught by the regex
                                    parts = user_input.split(',', 1)  # Split only on the first comma
                                    if len(parts) == 2:
                                        title_part = parts[0].replace('add', '', 1).replace('Add', '', 1).strip()
                                        description_part = parts[1].strip()
                                        entities['title'] = title_part
                                        entities['description'] = description_part
                        except IndexError:
